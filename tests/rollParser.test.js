...
var noGlobal = parse('{{dmg1=$[[3]]}} {{dmg1type=Slashing}} {{dmg2=$[[4]]}} {{dmg2type=Cold}}', inline);
expect('no global damage keeps ordinary entries', noGlobal.dmgRolls, [inline[3], inline[4]]);
expect('no global damage keeps ordinary types', noGlobal.dmgTypes, ['slashing', 'cold']);
var sneak = parse(fixtures.sneak.content, fixtures.sneak.inlinerolls);
expect('captured Sneak damage entries', sneak.dmgRolls, [fixtures.sneak.inlinerolls[2], fixtures.sneak.inlinerolls[3], fixtures.sneak.inlinerolls[6]]);
expect('Sneak label inherits piercing', sneak.dmgTypes, ['piercing', '', 'piercing']);
expect('real radiant rider type wins', parse(fixtures.sneak.content.replace('globaldamagetype=Sneak', 'globaldamagetype= Ra diant '), fixtures.sneak.inlinerolls).dmgTypes, ['piercing', '', 'radiant']);
var dueling = parse(fixtures.dueling.content, fixtures.dueling.inlinerolls);
expect('blank Dueling type inherits piercing', dueling.dmgTypes, ['piercing', '', 'piercing']);
expect('flat Dueling modifier preserved', dueling.dmgRolls[2], fixtures.dueling.inlinerolls[6]);
var upcast = parse(fixtures.upcast.content, fixtures.upcast.inlinerolls);
expect('documented higher level damage entries', upcast.dmgRolls, fixtures.upcast.inlinerolls.slice(1));
expect('higher level damage inherits fire', upcast.dmgTypes, ['fire', 'fire']);
var allFields = parse(fixtures.sneak.content + ' {{hldmg=$[[4]]}}', fixtures.sneak.inlinerolls);
expect('four damage sources in order', allFields.dmgRolls, [fixtures.sneak.inlinerolls[2], fixtures.sneak.inlinerolls[3], fixtures.sneak.inlinerolls[6], fixtures.sneak.inlinerolls[4]]);
expect('four damage types aligned', allFields.dmgTypes, ['piercing', '', 'piercing', 'piercing']);
